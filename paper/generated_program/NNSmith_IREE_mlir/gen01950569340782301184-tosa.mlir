module {
  func.func @main(%arg0: tensor<2x1x1x1xf32> {iree.identifier = "inputs_0"}, %arg1: tensor<2x1x1x1xf32> {iree.identifier = "inputs_1"}, %arg2: tensor<2x1x1x1xf32> {iree.identifier = "inputs_2"}) -> (tensor<2x1x1x1xf32> {iree.identifier = "Identity"}, tensor<2x1x3x1xf32> {iree.identifier = "Identity_1"}) {
    %0 = "tosa.const"() {value = dense<1.57079637> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %1 = "tosa.const"() {value = dense<0.159154937> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %2 = "tosa.const"() {value = dense<2.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %3 = "tosa.const"() {value = dense<1.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %4 = "tosa.const"() {value = dense<3.276700e+04> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %5 = "tosa.const"() {value = dense<2.38418579E-7> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %6 = "tosa.const"() {value = dense<3.80397034> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %7 = "tosa.const"() {value = dense<0.599528551> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %8 = "tosa.const"() {value = dense<""> : tensor<513xi16>} : () -> tensor<513xi16>
    %9 = "tosa.const"() {value = dense<[3, 1, 2, 0]> : tensor<4xi32>} : () -> tensor<4xi32>
    %10 = "tosa.add"(%arg0, %0) : (tensor<2x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x1x1xf32>
    %11 = "tosa.mul"(%10, %1) {shift = 0 : i32} : (tensor<2x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x1x1xf32>
    %12 = "tosa.floor"(%11) : (tensor<2x1x1x1xf32>) -> tensor<2x1x1x1xf32>
    %13 = "tosa.sub"(%11, %12) : (tensor<2x1x1x1xf32>, tensor<2x1x1x1xf32>) -> tensor<2x1x1x1xf32>
    %14 = "tosa.mul"(%13, %2) {shift = 0 : i32} : (tensor<2x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x1x1xf32>
    %15 = "tosa.sub"(%14, %3) : (tensor<2x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x1x1xf32>
    %16 = "tosa.mul"(%15, %4) {shift = 0 : i32} : (tensor<2x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x1x1xf32>
    %17 = "tosa.cast"(%16) : (tensor<2x1x1x1xf32>) -> tensor<2x1x1x1xi16>
    %18 = "tosa.table"(%17, %8) : (tensor<2x1x1x1xi16>, tensor<513xi16>) -> tensor<2x1x1x1xi32>
    %19 = "tosa.cast"(%18) : (tensor<2x1x1x1xi32>) -> tensor<2x1x1x1xf32>
    %20 = "tosa.mul"(%19, %5) {shift = 0 : i32} : (tensor<2x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x1x1xf32>
    %21 = "tosa.mul"(%arg1, %6) {shift = 0 : i32} : (tensor<2x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x1x1xf32>
    %22 = "tosa.add"(%21, %7) : (tensor<2x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x1x1xf32>
    %23 = "tosa.concat"(%arg2, %22, %20) {axis = 2 : i64} : (tensor<2x1x1x1xf32>, tensor<2x1x1x1xf32>, tensor<2x1x1x1xf32>) -> tensor<2x1x3x1xf32>
    %24 = "tf.Tan"(%22) {T = f32} : (tensor<2x1x1x1xf32>) -> tensor<2x1x1x1xf32>
    %25 = "tosa.transpose"(%23, %9) : (tensor<2x1x3x1xf32>, tensor<4xi32>) -> tensor<1x1x3x2xf32>
    %26 = "tosa.exp"(%25) : (tensor<1x1x3x2xf32>) -> tensor<1x1x3x2xf32>
    %27 = "tosa.reduce_sum"(%26) {axis = 3 : i64} : (tensor<1x1x3x2xf32>) -> tensor<1x1x3x1xf32>
    %28 = "tosa.reciprocal"(%27) : (tensor<1x1x3x1xf32>) -> tensor<1x1x3x1xf32>
    %29 = "tosa.mul"(%26, %28) {shift = 0 : i32} : (tensor<1x1x3x2xf32>, tensor<1x1x3x1xf32>) -> tensor<1x1x3x2xf32>
    %30 = "tosa.transpose"(%29, %9) : (tensor<1x1x3x2xf32>, tensor<4xi32>) -> tensor<2x1x3x1xf32>
    return %24, %30 : tensor<2x1x1x1xf32>, tensor<2x1x3x1xf32>
  }
}

