module {
  func.func @main(%arg0: tensor<2x2xf32> {iree.identifier = "inputs_0"}, %arg1: tensor<1x1x2x1xf32> {iree.identifier = "inputs_1"}) -> (tensor<1x1x2x2xf32> {iree.identifier = "Identity"}, tensor<1x1x2x1xf32> {iree.identifier = "Identity_1"}) {
    %0 = "tosa.const"() {value = dense<0.159154937> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %1 = "tosa.const"() {value = dense<2.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %2 = "tosa.const"() {value = dense<1.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %3 = "tosa.const"() {value = dense<3.276700e+04> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %4 = "tosa.const"() {value = dense<2.38418579E-7> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %5 = "tosa.const"() {value = dense<""> : tensor<513xi16>} : () -> tensor<513xi16>
    %6 = "tosa.const"() {value = dense<[[[[6.39311122]], [[3.61142015]]]]> : tensor<1x2x1x1xf32>} : () -> tensor<1x2x1x1xf32>
    %7 = "tosa.const"() {value = dense<6.10955142> : tensor<1xf32>} : () -> tensor<1xf32>
    %8 = "tosa.ceil"(%arg0) : (tensor<2x2xf32>) -> tensor<2x2xf32>
    %9 = "tosa.reshape"(%8) {new_shape = [1, 1, 2, 2]} : (tensor<2x2xf32>) -> tensor<1x1x2x2xf32>
    %10 = "tosa.add"(%arg1, %9) : (tensor<1x1x2x1xf32>, tensor<1x1x2x2xf32>) -> tensor<1x1x2x2xf32>
    %11 = "tosa.negate"(%10) : (tensor<1x1x2x2xf32>) -> tensor<1x1x2x2xf32>
    %12 = "tosa.reshape"(%11) {new_shape = [1, 2, 2, 1]} : (tensor<1x1x2x2xf32>) -> tensor<1x2x2x1xf32>
    %13 = "tosa.mul"(%10, %0) {shift = 0 : i32} : (tensor<1x1x2x2xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x2x2xf32>
    %14 = "tosa.floor"(%13) : (tensor<1x1x2x2xf32>) -> tensor<1x1x2x2xf32>
    %15 = "tosa.sub"(%13, %14) : (tensor<1x1x2x2xf32>, tensor<1x1x2x2xf32>) -> tensor<1x1x2x2xf32>
    %16 = "tosa.mul"(%15, %1) {shift = 0 : i32} : (tensor<1x1x2x2xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x2x2xf32>
    %17 = "tosa.sub"(%16, %2) : (tensor<1x1x2x2xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x2x2xf32>
    %18 = "tosa.mul"(%17, %3) {shift = 0 : i32} : (tensor<1x1x2x2xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x2x2xf32>
    %19 = "tosa.cast"(%18) : (tensor<1x1x2x2xf32>) -> tensor<1x1x2x2xi16>
    %20 = "tosa.table"(%19, %5) : (tensor<1x1x2x2xi16>, tensor<513xi16>) -> tensor<1x1x2x2xi32>
    %21 = "tosa.cast"(%20) : (tensor<1x1x2x2xi32>) -> tensor<1x1x2x2xf32>
    %22 = "tosa.mul"(%21, %4) {shift = 0 : i32} : (tensor<1x1x2x2xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x2x2xf32>
    %23 = "tosa.conv2d"(%12, %6, %7) {dilation = [1, 1], pad = [0, 0, 0, 0], stride = [1, 1]} : (tensor<1x2x2x1xf32>, tensor<1x2x1x1xf32>, tensor<1xf32>) -> tensor<1x1x2x1xf32>
    return %22, %23 : tensor<1x1x2x2xf32>, tensor<1x1x2x1xf32>
  }
}

