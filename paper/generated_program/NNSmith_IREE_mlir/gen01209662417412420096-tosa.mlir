module {
  func.func @main(%arg0: tensor<1xf32> {iree.identifier = "inputs_0"}, %arg1: tensor<1x1x2x1xf32> {iree.identifier = "inputs_1"}) -> (tensor<1x2x2x1xf32> {iree.identifier = "Identity"}) {
    %0 = "tosa.const"() {value = dense<0.159154937> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %1 = "tosa.const"() {value = dense<2.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %2 = "tosa.const"() {value = dense<1.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %3 = "tosa.const"() {value = dense<3.276700e+04> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %4 = "tosa.const"() {value = dense<2.38418579E-7> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %5 = "tosa.const"() {value = dense<""> : tensor<513xi16>} : () -> tensor<513xi16>
    %6 = "tosa.const"() {value = dense<[[0, 0], [1, 1], [1, 1], [0, 0]]> : tensor<4x2xi32>} : () -> tensor<4x2xi32>
    %7 = "tosa.const"() {value = dense<0xFF800000> : tensor<f32>} : () -> tensor<f32>
    %8 = "tosa.reshape"(%arg0) {new_shape = [1, 1, 1, 1]} : (tensor<1xf32>) -> tensor<1x1x1x1xf32>
    %9 = "tosa.mul"(%8, %arg1) {shift = 0 : i32} : (tensor<1x1x1x1xf32>, tensor<1x1x2x1xf32>) -> tensor<1x1x2x1xf32>
    %10 = "tosa.pad"(%9, %6, %7) : (tensor<1x1x2x1xf32>, tensor<4x2xi32>, tensor<f32>) -> tensor<1x3x4x1xf32>
    %11 = "tosa.max_pool2d"(%10) {kernel = [2, 3], pad = [0, 0, 0, 0], stride = [1, 1]} : (tensor<1x3x4x1xf32>) -> tensor<1x2x2x1xf32>
    %12 = "tosa.mul"(%11, %0) {shift = 0 : i32} : (tensor<1x2x2x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x2x2x1xf32>
    %13 = "tosa.floor"(%12) : (tensor<1x2x2x1xf32>) -> tensor<1x2x2x1xf32>
    %14 = "tosa.sub"(%12, %13) : (tensor<1x2x2x1xf32>, tensor<1x2x2x1xf32>) -> tensor<1x2x2x1xf32>
    %15 = "tosa.mul"(%14, %1) {shift = 0 : i32} : (tensor<1x2x2x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x2x2x1xf32>
    %16 = "tosa.sub"(%15, %2) : (tensor<1x2x2x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x2x2x1xf32>
    %17 = "tosa.mul"(%16, %3) {shift = 0 : i32} : (tensor<1x2x2x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x2x2x1xf32>
    %18 = "tosa.cast"(%17) : (tensor<1x2x2x1xf32>) -> tensor<1x2x2x1xi16>
    %19 = "tosa.table"(%18, %5) : (tensor<1x2x2x1xi16>, tensor<513xi16>) -> tensor<1x2x2x1xi32>
    %20 = "tosa.cast"(%19) : (tensor<1x2x2x1xi32>) -> tensor<1x2x2x1xf32>
    %21 = "tosa.mul"(%20, %4) {shift = 0 : i32} : (tensor<1x2x2x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x2x2x1xf32>
    %22 = "tosa.reciprocal"(%9) : (tensor<1x1x2x1xf32>) -> tensor<1x1x2x1xf32>
    %23 = "tosa.mul"(%21, %22) {shift = 0 : i32} : (tensor<1x2x2x1xf32>, tensor<1x1x2x1xf32>) -> tensor<1x2x2x1xf32>
    return %23 : tensor<1x2x2x1xf32>
  }
}

