module {
  func.func @main(%arg0: tensor<1x1x1x1xf32> {iree.identifier = "inputs_0"}) -> (tensor<1x1x1x1xf32> {iree.identifier = "Identity"}, tensor<1x1x1xi64> {iree.identifier = "Identity_1"}, tensor<1x1xi64> {iree.identifier = "Identity_2"}) {
    %0 = "tosa.const"() {value = dense<1.57079637> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %1 = "tosa.const"() {value = dense<0.159154937> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %2 = "tosa.const"() {value = dense<2.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %3 = "tosa.const"() {value = dense<1.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %4 = "tosa.const"() {value = dense<3.276700e+04> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %5 = "tosa.const"() {value = dense<2.38418579E-7> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %6 = "tosa.const"() {value = dense<""> : tensor<513xi16>} : () -> tensor<513xi16>
    %7 = "tosa.add"(%arg0, %0) : (tensor<1x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x1x1xf32>
    %8 = "tosa.mul"(%7, %1) {shift = 0 : i32} : (tensor<1x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x1x1xf32>
    %9 = "tosa.floor"(%8) : (tensor<1x1x1x1xf32>) -> tensor<1x1x1x1xf32>
    %10 = "tosa.sub"(%8, %9) : (tensor<1x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x1x1xf32>
    %11 = "tosa.mul"(%10, %2) {shift = 0 : i32} : (tensor<1x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x1x1xf32>
    %12 = "tosa.sub"(%11, %3) : (tensor<1x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x1x1xf32>
    %13 = "tosa.mul"(%12, %4) {shift = 0 : i32} : (tensor<1x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x1x1xf32>
    %14 = "tosa.cast"(%13) : (tensor<1x1x1x1xf32>) -> tensor<1x1x1x1xi16>
    %15 = "tosa.table"(%14, %6) : (tensor<1x1x1x1xi16>, tensor<513xi16>) -> tensor<1x1x1x1xi32>
    %16 = "tosa.cast"(%15) : (tensor<1x1x1x1xi32>) -> tensor<1x1x1x1xf32>
    %17 = "tosa.mul"(%16, %5) {shift = 0 : i32} : (tensor<1x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x1x1x1xf32>
    %18 = "tosa.floor"(%17) : (tensor<1x1x1x1xf32>) -> tensor<1x1x1x1xf32>
    %19 = "tosa.argmax"(%17) {axis = 3 : i64} : (tensor<1x1x1x1xf32>) -> tensor<1x1x1xi64>
    %20 = "tosa.reshape"(%19) {new_shape = [1, 1]} : (tensor<1x1x1xi64>) -> tensor<1x1xi64>
    %21 = "tosa.argmax"(%17) {axis = 1 : i64} : (tensor<1x1x1x1xf32>) -> tensor<1x1x1xi64>
    return %18, %21, %20 : tensor<1x1x1x1xf32>, tensor<1x1x1xi64>, tensor<1x1xi64>
  }
}

