module {
  func.func @main(%arg0: tensor<2x1x2x1xf32> {iree.identifier = "inputs_0"}, %arg1: tensor<?xf32> {iree.identifier = "inputs_1"}) -> (tensor<2x1x2x?xi1> {iree.identifier = "Identity"}, tensor<2x1x2x?xf32> {iree.identifier = "Identity_1"}) {
    %0 = "tosa.const"() {value = dense<1.57079637> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %1 = "tosa.const"() {value = dense<0.159154937> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %2 = "tosa.const"() {value = dense<2.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %3 = "tosa.const"() {value = dense<1.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %4 = "tosa.const"() {value = dense<3.276700e+04> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %5 = "tosa.const"() {value = dense<2.38418579E-7> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %6 = "tosa.const"() {value = dense<""> : tensor<513xi16>} : () -> tensor<513xi16>
    %7 = "tosa.sub"(%arg1, %arg0) : (tensor<?xf32>, tensor<2x1x2x1xf32>) -> tensor<2x1x2x?xf32>
    %8 = "tosa.add"(%7, %0) : (tensor<2x1x2x?xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x2x?xf32>
    %9 = "tosa.mul"(%8, %1) {shift = 0 : i32} : (tensor<2x1x2x?xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x2x?xf32>
    %10 = "tosa.floor"(%9) : (tensor<2x1x2x?xf32>) -> tensor<2x1x2x?xf32>
    %11 = "tosa.sub"(%9, %10) : (tensor<2x1x2x?xf32>, tensor<2x1x2x?xf32>) -> tensor<2x1x2x?xf32>
    %12 = "tosa.mul"(%11, %2) {shift = 0 : i32} : (tensor<2x1x2x?xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x2x?xf32>
    %13 = "tosa.sub"(%12, %3) : (tensor<2x1x2x?xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x2x?xf32>
    %14 = "tosa.mul"(%13, %4) {shift = 0 : i32} : (tensor<2x1x2x?xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x2x?xf32>
    %15 = "tosa.cast"(%14) : (tensor<2x1x2x?xf32>) -> tensor<2x1x2x?xi16>
    %16 = "tosa.table"(%15, %6) : (tensor<2x1x2x?xi16>, tensor<513xi16>) -> tensor<2x1x2x?xi32>
    %17 = "tosa.cast"(%16) : (tensor<2x1x2x?xi32>) -> tensor<2x1x2x?xf32>
    %18 = "tosa.mul"(%17, %5) {shift = 0 : i32} : (tensor<2x1x2x?xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x2x?xf32>
    %19 = "tosa.mul"(%18, %1) {shift = 0 : i32} : (tensor<2x1x2x?xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x2x?xf32>
    %20 = "tosa.floor"(%19) : (tensor<2x1x2x?xf32>) -> tensor<2x1x2x?xf32>
    %21 = "tosa.sub"(%19, %20) : (tensor<2x1x2x?xf32>, tensor<2x1x2x?xf32>) -> tensor<2x1x2x?xf32>
    %22 = "tosa.mul"(%21, %2) {shift = 0 : i32} : (tensor<2x1x2x?xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x2x?xf32>
    %23 = "tosa.sub"(%22, %3) : (tensor<2x1x2x?xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x2x?xf32>
    %24 = "tosa.mul"(%23, %4) {shift = 0 : i32} : (tensor<2x1x2x?xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x2x?xf32>
    %25 = "tosa.cast"(%24) : (tensor<2x1x2x?xf32>) -> tensor<2x1x2x?xi16>
    %26 = "tosa.table"(%25, %6) : (tensor<2x1x2x?xi16>, tensor<513xi16>) -> tensor<2x1x2x?xi32>
    %27 = "tosa.cast"(%26) : (tensor<2x1x2x?xi32>) -> tensor<2x1x2x?xf32>
    %28 = "tosa.mul"(%27, %5) {shift = 0 : i32} : (tensor<2x1x2x?xf32>, tensor<1x1x1x1xf32>) -> tensor<2x1x2x?xf32>
    %29 = "tosa.maximum"(%18, %28) : (tensor<2x1x2x?xf32>, tensor<2x1x2x?xf32>) -> tensor<2x1x2x?xf32>
    %30 = "tosa.greater"(%18, %7) : (tensor<2x1x2x?xf32>, tensor<2x1x2x?xf32>) -> tensor<2x1x2x?xi1>
    return %30, %29 : tensor<2x1x2x?xi1>, tensor<2x1x2x?xf32>
  }
}

