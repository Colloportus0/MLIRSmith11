module {
  func.func @main(%arg0: tensor<1x1x1x1xf32> {iree.identifier = "inputs_0"}, %arg1: tensor<1x1x1x1xf32> {iree.identifier = "inputs_1"}, %arg2: tensor<1x1x1x1xf32> {iree.identifier = "inputs_2"}) -> (tensor<1x3x1xf32> {iree.identifier = "Identity"}, tensor<1x3x1x1xf32> {iree.identifier = "Identity_1"}) {
    %0 = "tosa.const"() {value = dense<0.00999999977> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %1 = "tosa.const"() {value = dense<0.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %2 = "tosa.const"() {value = dense<1.57079637> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %3 = "tosa.const"() {value = dense<0.159154937> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %4 = "tosa.const"() {value = dense<2.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %5 = "tosa.const"() {value = dense<1.000000e+00> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %6 = "tosa.const"() {value = dense<3.276700e+04> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %7 = "tosa.const"() {value = dense<2.38418579E-7> : tensor<1x1x1x1xf32>} : () -> tensor<1x1x1x1xf32>
    %8 = "tosa.const"() {value = dense<""> : tensor<513xi16>} : () -> tensor<513xi16>
    %9 = "tosa.concat"(%arg2, %arg1, %arg0) {axis = 1 : i64} : (tensor<1x1x1x1xf32>, tensor<1x1x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x3x1x1xf32>
    %10 = "tosa.mul"(%9, %0) {shift = 0 : i32} : (tensor<1x3x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x3x1x1xf32>
    %11 = "tosa.greater_equal"(%9, %1) : (tensor<1x3x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x3x1x1xi1>
    %12 = "tosa.select"(%11, %9, %10) : (tensor<1x3x1x1xi1>, tensor<1x3x1x1xf32>, tensor<1x3x1x1xf32>) -> tensor<1x3x1x1xf32>
    %13 = "tosa.abs"(%12) : (tensor<1x3x1x1xf32>) -> tensor<1x3x1x1xf32>
    %14 = "tosa.add"(%13, %2) : (tensor<1x3x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x3x1x1xf32>
    %15 = "tosa.mul"(%14, %3) {shift = 0 : i32} : (tensor<1x3x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x3x1x1xf32>
    %16 = "tosa.floor"(%15) : (tensor<1x3x1x1xf32>) -> tensor<1x3x1x1xf32>
    %17 = "tosa.sub"(%15, %16) : (tensor<1x3x1x1xf32>, tensor<1x3x1x1xf32>) -> tensor<1x3x1x1xf32>
    %18 = "tosa.mul"(%17, %4) {shift = 0 : i32} : (tensor<1x3x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x3x1x1xf32>
    %19 = "tosa.sub"(%18, %5) : (tensor<1x3x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x3x1x1xf32>
    %20 = "tosa.mul"(%19, %6) {shift = 0 : i32} : (tensor<1x3x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x3x1x1xf32>
    %21 = "tosa.cast"(%20) : (tensor<1x3x1x1xf32>) -> tensor<1x3x1x1xi16>
    %22 = "tosa.table"(%21, %8) : (tensor<1x3x1x1xi16>, tensor<513xi16>) -> tensor<1x3x1x1xi32>
    %23 = "tosa.cast"(%22) : (tensor<1x3x1x1xi32>) -> tensor<1x3x1x1xf32>
    %24 = "tosa.mul"(%23, %7) {shift = 0 : i32} : (tensor<1x3x1x1xf32>, tensor<1x1x1x1xf32>) -> tensor<1x3x1x1xf32>
    %25 = "tosa.reshape"(%9) {new_shape = [1, 3, 1]} : (tensor<1x3x1x1xf32>) -> tensor<1x3x1xf32>
    return %25, %24 : tensor<1x3x1xf32>, tensor<1x3x1x1xf32>
  }
}

